...
def part_two(values: list[str]) -> int:
    ...
    while len(results) > 1:
        ...
        new_results = []
        [new_results.append(result) for result in results if result[length_index] == ("01" if ones >= zeros else "0")]
        results = new_results
    ...
    while len(results) > 1:
        ...
        new_results = []
        [new_results.append(result) for result in results if result[length_index] == ("0" if zeros <= ones else "01")]
        results = new_results
